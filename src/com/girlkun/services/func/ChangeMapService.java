...
public class ChangeMapService {
    ...
    public void changeZone(Player pl, int zoneId) {
        ...
        if (!pl.isAdmin() || !pl.isBoss) {
            ...
            if (MapService.gI().isMapthapkarin(pl.zone.map.mapId)) {
                Service.gI().sendThongBaoOK(pl, "Không thể đổi khu vực trong map này");
                return;
            }

            if (MapService.gI().isMapLeoThap(pl.zone.map.mapId)) {
                Service.gI().sendThongBaoOK(pl, "Không thể đổi khu vực trong map này");
                return;
            }
            if (MapService.gI().isMapBanDoKhoBau(pl.zone.map.mapId)) {
    ...
    public void goHomefromDT(Player player) {
        if (!player.iDMark.isGoToHomefromDT()) {
            player.iDMark.setLastTimeGoToHomefromDT(System.currentTimeMillis());
            player.iDMark.setGoToHomefromDT(true);
    ...
    private void changeMap(Player pl, Zone zoneJoin, int mapId, int zoneId, int x, int y, byte typeSpace) {
        ...
        if (zoneJoin != null) {
            boolean currMapIsCold = false;
            boolean nextMapIsCold = false;
            if (!pl.isBoss) {
                currMapIsCold = MapService.gI().isMapCold(pl.zone.map);
                nextMapIsCold = MapService.gI().isMapCold(zoneJoin.map);
            }
            ...
            if (!pl.isBoss && !pl.isPet && !pl.isPet3 && !pl.isNewPet && !pl.isfake) {
        ...
        try {
            for (Mob mob : player.zone.mobs) {
                ...
                if (mob.effectSkill.isSocola) {
                    ...
                    player.sendMessage(msg);
                    msg.cleanup();
                } else if (mob.MobImage != 0) {
                    msg = new Message(-112);
                    msg.writer().writeByte(1);
                    msg.writer().writeByte(mob.id); //b4
                    msg.writer().writeShort(mob.MobImage);//b5
                    player.sendMessage(msg);
                    msg.cleanup();
                }
    ...
    public void getNewMob(Mob mob, Player player) {
        Message msg;
        try {
            msg = new Message(-112);
            msg.writer().writeByte(1);
            msg.writer().writeByte(mob.id); //b4
            msg.writer().writeShort(getImgMob(mob));//b5
            player.sendMessage(msg);
            msg.cleanup();
        } catch (Exception e) {
