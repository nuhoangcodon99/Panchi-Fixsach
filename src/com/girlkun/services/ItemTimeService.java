...
public class ItemTimeService {
    ...
    public void sendAllItemTime(Player player) {
        ...
        if (player.itemTime.isUseMayDo) {
         ...
         if(player.effectSkill.isbroly && player.effectSkill.lastTimeUpBroly>=0) {
               sendItemTime(player, 21839, (int) ((300000 - (System.currentTimeMillis() - player.effectSkill.lastTimeUpBroly)) / 1000));
        }
    ...
    public void sendTextDoanhTrai(Player player) {
        if (player.clan != null && !player.clan.haveGoneDoanhTrai && player.clan.timeOpenDoanhTrai != 0L) {
            ...
            this.sendTextTime(player, (byte) 0, "Doanh trại độc nhãn: ", secondsLeft);
        }
