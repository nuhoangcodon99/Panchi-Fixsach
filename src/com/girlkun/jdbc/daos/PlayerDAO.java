...
public class PlayerDAO {
    ...
    public static boolean createNewPlayer(int userId, String name, byte gender, int hair) {
        try {
            ...
            dataArray.add(200000); //hồng ngọc
            ...
            for (int i = 0; i < 80; i++) { // item tạo player 
                if (i == 0) { //Capsule Vip
            ...
            GirlkunDB.executeUpdate("insert into player"
                    ...
                    + "data_black_ball, data_side_task, data_card, bill_data, info_phoban, data_may_do, info_achievement, congduc, data_offtrain) "
                    ...
                    itemTime, itemTimeSC, task, mabuEgg, charms, skills, skillsShortcut, petData, dataBlackBall, dataSideTask, data_card, bill_data, info_phoban, itemMayDo, info_achive, 0, dataoff);
            ...
            try {
                ...
                if (player.isDie()) {
                    ...
                    if (MapService.gI().isMapDoanhTrai(mapId) || MapService.gI().isMapBlackBallWar(mapId)
                            ...
                            || MapService.gI().isMapGiaiCuuMiNuong(mapId)) {
                        mapId = player.gender + 21;
                        x = 300;
                        y = 336;
                    }
                ...
                achievementObject.put("sm1", 0);
                achievementObject.put("sm2", 0);
                achievementObject.put("sm3", 0);
                achievementObject.put("sm4", 0);
                achievementObject.put("sm5", 0);
                achievementObject.put("sm6", 0);
                achievementObject.put("sm7", 0);
                achievementObject.put("sm8", 0);
                achievementObject.put("sm9", 0);
                achievementObject.put("sm10", 0);
                ...
                String query = " update player set info_achievement =?, data_may_do = ?, khi_gas =?, data_item_time_sieu_cap = ?,head = ?, have_tennis_space_ship = ?,"
                        ...
                        + " skills_shortcut = ?, pointPvp=?, NguHanhSonPoint=?, xuatsu=?, damethanmeo=?, ptdame=?, diemtrungthu=?, diembitich=?,data_card=?,bill_data =? ,info_phoban =?, congduc =?, data_offtrain =?, capboss=?, point_gapthu =?, point_cauca =?, thoigianduhanh = ?, thachdauwhis = ? where id = ?";
