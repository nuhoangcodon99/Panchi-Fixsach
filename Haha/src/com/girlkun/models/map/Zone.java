...
//                msg.writer().writeShort(0);
            try {
                ...
                byte[] effngay = FileIO.readFile("data/girlkun/map/eff_map/" + this.map.mapId);
                byte[] effdem = FileIO.readFile("data/girlkun/map/eff_map_dem/" + this.map.mapId);
                if (pl.isdem) {
                    msg.writer().write(effdem);
                } else {
                    msg.writer().write(effngay);
                }
            } catch (Exception e) {
                msg.writer().writeShort(0);
            }
