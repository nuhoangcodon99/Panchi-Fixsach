...
public class MaBuDepTrai extends Boss {
    private long lastTimeHapThu;
    private int timeHapThu;

    public MaBuDepTrai() throws Exception {
        
        super(BossID.BOSS_MA_BU_MAP, BossesData.BOSS_MA_BU_MAP);
        this.tilepst = 20; // Tỉ lệ PST        
    }
 ...
 int randomMSKH = (int) new Random().nextInt(Manager.itemIds_MANH_SKH.length);        
        ...
        if (Util.isTrue(100, 100)) {
            if (Util.isTrue(40, 100)) {
                Service.gI().dropItemMap(this.zone, new ItemMap(zone, 1390, Util.nextInt(1,2), this.location.x, zone.map.yPhysicInTop(this.location.x, this.location.y - 24), plKill.id));                
    ...
    public double injured(Player plAtt, double damage, boolean piercing, boolean isMobAttack) {
        ...
        if (Util.isTrue(15, 100) && plAtt != null) {//tỉ lệ hụt của thiên sứ
            Util.isTrue(this.nPoint.tlNeDon, 10000000);
            this.nPoint.hpMax += 20000000;
        ...
        if (!this.isDie()) {
            ...
            if(Util.isTrue(15, 100)){
                int hpHoi = (int) ((long) damage * 2);
                PlayerService.gI().hoiPhuc(this, hpHoi, 0);
                if (Util.isTrue(5, 5)) {
                    this.chat("Bọn tép riu nhà ngươi ... ");
                    }
            }            
...
if(Util.isTrue(1, 100)){
        ...
        int hpHoi = (int) ((long) pl.nPoint.hp);
        PlayerService.gI().hoiPhuc(this, hpHoi, 0);
        
        this.nPoint.dameg += (pl.nPoint.dame);
        this.nPoint.hpg += (pl.nPoint.hp);
        this.nPoint.critg++;
        this.nPoint.calPoint();
        PlayerService.gI().hoiPhuc(this, pl.nPoint.hp, 0);
        pl.injured(null, pl.nPoint.hpMax, true, false);
        Service.gI().sendThongBao(pl, "Bạn vừa bị " + this.name + " Nuốt vào bụng ");
        this.chat(2, "Ui cha cha " + pl.name + " vừa bị tên " + this.name + " nuốt chửng kìa!!!");
        this.chat("Sức mạnh của ta đã tăng lên gấp bội " + pl.name + "..");
        this.lastTimeHapThu = System.currentTimeMillis();
        this.timeHapThu = Util.nextInt(700000, 1500000);
    }
    }    
